...
export function PipelineSettings({
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t("pipelines.settings.stageHasDealsError"));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t("pipelines.settings.deletePipelineFailed"));
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
        ...
        {showDeleteConfirm ? (
          <div className="py-4">
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-red-400">
                  {t("pipelines.settings.deletePipelineTitle")}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("pipelines.settings.deletePipelineDesc")}
                </p>
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {deleting ? t("common.deleting") : t("pipelines.settings.deletePipelineTitle")}
              </Button>
            </div>
          </div>
        ) : (
          <>
            <div className="grid gap-4 py-2">
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("pipelines.settings.pipelineNameLabel")}</Label>
                <Input
                  value={name}
              ...
              <div className="grid gap-2">
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {STAGE_COLORS.map((color) => (
                    <button
                      ...
                      aria-label={t("pipelines.settings.pickColorAria").replace("{color}", color)}
                    />
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    placeholder={t("pipelines.settings.newStageNamePh")}
