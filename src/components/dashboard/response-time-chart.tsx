...
// `{ day: 'Mon', [category]: 4.2 }` rows below. The category label
// itself is translated, so it's resolved inside the component.
...
export function ResponseTimeChart({
  ...
  const { t } = useLocale()
  const CATEGORY = t('dash.chart.responseTime.avgMinutes')
    ...
    <section className="rounded-xl border border-border bg-card">
      <header className="flex items-center justify-between gap-3 border-b border-border px-5 py-4">
        <div>
          <h2 className="text-sm font-semibold text-foreground">
            {t('dash.chart.responseTime.title')}
          </h2>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {t('dash.chart.responseTime.subtitle')}
          </p>
        </div>
        <div className="flex items-center gap-3 text-right text-xs">
          {thresholdMinutes > 0 && (
            <span className="rounded-full border border-rose-500/40 bg-rose-500/10 px-2 py-0.5 font-medium text-rose-300 tabular-nums">
              {t('dash.chart.responseTime.target').replace('{minutes}', String(thresholdMinutes))}
            </span>
          )}
          {data && (data.thisWeekAvg != null || data.lastWeekAvg != null) && (
            <div>
              <div className="text-muted-foreground">
                {t('dash.chart.responseTime.thisWeek')}{' '}
                <span className="font-medium text-foreground tabular-nums">
                  {fmt(data.thisWeekAvg)}
                </span>
              </div>
              <div className="text-muted-foreground">
                {t('dash.chart.responseTime.lastWeek')}{' '}
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={Clock}
            title={t('dash.chart.responseTime.emptyTitle')}
            hint={t('dash.chart.responseTime.emptyHint')}
          />
        ) : (
          <BarChart
            data={chartData}
