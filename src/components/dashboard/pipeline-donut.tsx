  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card">
      <header className="border-b border-border px-5 py-4">
        <h2 className="text-sm font-semibold text-foreground">{t('dash.chart.pipeline.title')}</h2>
        <p className="mt-0.5 text-xs text-muted-foreground">
          {t('dash.chart.pipeline.subtitle')}
        </p>
      </header>
      ...
      <div className="flex flex-1 flex-col p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={GitBranch}
            title={t('dash.chart.pipeline.emptyTitle')}
            hint={t('dash.chart.pipeline.emptyHint')}
          />
        ) : (
          <>
            <Donut data={data} currency={currency} totalLabel={t('dash.chart.pipeline.total')} ariaLabel={t('dash.chart.pipeline.ariaLabel')} />
              ...
              {data.stages.map((s) => (
                <li key={s.id} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="text-muted-foreground tabular-nums">
                    {s.dealCount} {s.dealCount === 1 ? t('dash.chart.pipeline.dealSingular') : t('dash.chart.pipeline.dealPlural')}
                  </span>
