...
// Rótulos resolvidos via i18n (invite.day<value>).
...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    if (trimmedLabel.length > MAX_LABEL_LEN) {
      toast.error(
        t('invite.toast.labelTooLong').replace('{max}', String(MAX_LABEL_LEN)),
      );
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('invite.toast.createFailed'));
        return;
      }
      ...
      setResult({
        ...
        accountName: account?.name ?? t('invite.defaultAccountName'),
      });
      onCreated();
    } catch (err) {
      console.error('[InviteMemberDialog] create error:', err);
      toast.error(t('invite.toast.serverUnreachable'));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function copyToClipboard() {
    ...
    try {
      ...
      // hand-copy it.
      toast.error(t('invite.toast.clipboardBlocked'));
    }
  }
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where the default fallback
    // name wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? t('invite.defaultAccountName');
    const message = t('invite.waMessage')
    ...
    >
      <DialogContent className="bg-popover border-border sm:max-w-md">
        {result ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                {t('invite.createdDesc')
                  .replace('{role}', t(`role.${result.role}`))
                  .replace('{days}', String(result.expiresInDays))}
              </DialogDescription>
            </DialogHeader>
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">{t('invite.title')}</DialogTitle>
              <DialogDescription className="text-muted-foreground">
                {t('invite.desc')}
              </DialogDescription>
            </DialogHeader>
