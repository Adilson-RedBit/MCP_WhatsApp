...
export function ProfileForm() {
  ...
  const onPickFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_MIME.has(file.type)) {
      toast.error(t('profile.toast.unsupportedType'), {
        description: t('profile.toast.unsupportedTypeDesc'),
      });
      return;
    }
    if (file.size > MAX_AVATAR_BYTES) {
      toast.error(t('profile.toast.tooLarge'), {
        description: t('profile.toast.tooLargeDesc'),
      });
      return;
    }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedName) {
      toast.error(t('profile.toast.nameRequired'));
      return;
    }
    const trimmedEmail = email.trim();
    if (!EMAIL_RE.test(trimmedEmail)) {
      toast.error(t('profile.toast.invalidEmail'));
      return;
    }
    ...
    try {
      ...
      if (pendingAvatar) {
        ...
        if (uploadError) {
          throw new Error(
            `${t('profile.toast.uploadFailedPrefix')} ${uploadError.message}`,
          );
        }
        const {
          data: { publicUrl },
      ...
      if (updateError) {
        throw new Error(
          `${t('profile.toast.saveFailedPrefix')} ${updateError.message}`,
        );
      }
      ...
      if (trimmedEmail.toLowerCase() !== profile.email.toLowerCase()) {
        ...
        if (emailError) {
          ...
          toast.error(
            `${t('profile.toast.emailChangeFailedPrefix')} ${emailError.message}`,
          );
          setSaving(false);
      ...
      toast.success(
        emailSent
          ? t('profile.toast.savedEmailPending')
          : t('profile.toast.saved'),
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : t('profile.toast.unknownError');
      toast.error(msg);
    } finally {
      setSaving(false);
    }
        ...
        <Card>
          ...
          <div className="flex flex-wrap items-center gap-5">
            <Avatar size="lg" className="size-16">
              {currentAvatar ? (
                <AvatarImage src={currentAvatar} alt={fullName || t('profile.avatarAlt')} />
              ) : null}
