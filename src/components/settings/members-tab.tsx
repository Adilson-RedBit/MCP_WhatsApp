...
// Labels resolve via i18n keys (role.<value>).
...
function fmtExpiresIn(iso: string, t: (key: string) => string): string {
  const ms = new Date(iso).getTime() - Date.now();
  if (ms <= 0) return t('members.expired');
  const days = Math.floor(ms / (24 * 60 * 60 * 1000));
  if (days >= 1) {
    return (days === 1 ? t('members.expiresInDay') : t('members.expiresInDays')).replace(
      '{days}',
      String(days),
    );
  }
  const hours = Math.max(1, Math.floor(ms / (60 * 60 * 1000)));
  return (hours === 1 ? t('members.expiresInHour') : t('members.expiresInHours')).replace(
    '{hours}',
    String(hours),
  );
}
...
export function MembersTab() {
  ...
  const loadEverything = useCallback(async () => {
    try {
      ...
      if (!mres.ok) {
        ...
        toast.error(payload.error || t('members.toast.loadMembersFailed'));
        return;
      }
      ...
      if (ires) {
        if (!ires.ok) {
          ...
          toast.error(payload.error || t('members.toast.loadInvitationsFailed'));
          return;
        }
      ...
      toast.error(t('members.toast.serverUnreachable'));
    } finally {
      setLoading(false);
    }
  }, [canManageMembers]);
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('members.toast.updateRoleFailed'));
        return;
      }
      toast.success(
        t('members.toast.roleUpdated')
          .replace('{name}', member.full_name || t('members.unnamed'))
          .replace('{role}', t(`role.${nextRole}`)),
      );
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('members.toast.removeMemberFailed'));
        return;
      }
      toast.success(
        t('members.toast.memberRemoved').replace(
          '{name}',
          removingMember.full_name || t('members.unnamed'),
        ),
      );
      setMembers((prev) =>
  ...
  async function handleRevoke(invite: Invitation) {
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('members.toast.revokeInviteFailed'));
        return;
      }
          ...
          return (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              ...
              <span className="text-muted-foreground/70">
                · {members.length}{' '}
                {members.length === 1 ? t('members.count') : t('members.countPlural')}
              </span>
            </div>
          );
        })()}
                          ...
                          <Avatar className="size-9 shrink-0">
                            {member.avatar_url ? (
                              <AvatarImage
                                ...
                                alt={member.full_name || t('members.avatarAlt')}
                              />
                            ) : null}
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          {t('members.createdAt')} {fmtDate(inv.created_at)} · {fmtExpiresIn(inv.expires_at, t)}
                        </p>
                      </div>
