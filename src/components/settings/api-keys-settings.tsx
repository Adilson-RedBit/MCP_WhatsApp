...
export function ApiKeysSettings() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('api.toast.loadFailed'));
        return;
      }
      ...
      toast.error(t('api.toast.serverUnreachable'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handleRevoke(key: ApiKey) {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('api.toast.revokeFailed'));
        return;
      }
      toast.success(t('api.toast.keyRevoked').replace('{name}', key.name));
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-muted-foreground mt-1.5 text-xs">
                        {t('api.created')} {fmtDate(k.created_at)}
                        {' · '}
                        {k.last_used_at
                          ? `${t('api.lastUsed')} ${fmtDate(k.last_used_at)}`
                          : t('api.neverUsed')}
                        {k.expires_at && status !== 'expired'
                          ? ` · ${t('api.expires')} ${fmtDate(k.expires_at)}`
                          : ''}
                      </p>
                    </div>
...
function CreateKeyDialog({
  ...
  async function handleCreate() {
    ...
    if (!trimmed) {
      toast.error(t('api.toast.nameRequired'));
      return;
    }
    setSubmitting(true);
    try {
      ...
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(payload.error || t('api.toast.createFailed'));
        return;
      }
  ...
  async function copyKey() {
    ...
    try {
      ...
      toast.success(t('api.toast.keyCopied'));
    } catch {
      toast.error(t('api.toast.copyFailed'));
    }
  }
        ...
        {createdKey ? (
          <>
            ...
            <div className="space-y-1.5">
              <Label className="text-muted-foreground">{t('api.key')}</Label>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  value={name}
                  maxLength={80}
                  placeholder={t('api.namePh')}
                  onChange={(e) => setName(e.target.value)}
                />
              </div>

              <div className="space-y-2">
                <Label className="text-muted-foreground">{t('api.scopes')}</Label>
