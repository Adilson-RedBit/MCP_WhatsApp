  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t('cf.title')}</DialogTitle>
...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (isDuplicate(name)) {
      toast.error(t('contacts.customFields.duplicateName').replace('{name}', name));
      return;
    }
    ...
    toast.success(t('contacts.customFields.created').replace('{name}', name));
    setNewName('');
    await fetchFields();
  }
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        t('contacts.customFields.deleteConfirm').replace('{name}', field.field_name)
      )
    ) {
      return;
    }
    ...
    toast.success(t('contacts.customFields.deleted').replace('{name}', field.field_name));
    await fetchFields();
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          placeholder={t('contacts.customFields.namePh')}
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      <Input
        ...
        aria-label={t('contacts.customFields.renameAria').replace('{name}', field.field_name)}
