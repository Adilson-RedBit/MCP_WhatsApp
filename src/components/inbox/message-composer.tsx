...
/** Maps a composer media kind to the i18n key holding its translated noun
 *  (shared with the message-bubble labels so the words stay consistent). */
const MEDIA_KIND_LABEL_KEY: Record<ComposerMediaKind, string> = {
  image: "inbox.bubble.image",
  video: "inbox.bubble.video",
  document: "inbox.bubble.document",
  audio: "inbox.bubble.audio",
};
...
export function MessageComposer({
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      if (file.size > max) {
        const kindLabel = t(MEDIA_KIND_LABEL_KEY[kind]).toLowerCase();
        toast.error(
          t("inbox.composer.fileTooLarge")
            .replace("{size}", (file.size / 1024 / 1024).toFixed(1))
            .replace("{kind}", kindLabel)
            .replace("{max}", String(Math.round(max / 1024 / 1024))),
        );
        return;
      }
      setBusy(true);
  ...
  const startRecording = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof AudioContext === "undefined") {
      toast.error(t("inbox.composer.voiceNotSupported"));
      return;
    }
    try {
      ...
      {draft ? (
        ...
        <div className="flex items-end gap-2">
          ...
          <GatedButton
            ...
            title={readOnly ? undefined : t("inbox.template.sendTemplate")}
