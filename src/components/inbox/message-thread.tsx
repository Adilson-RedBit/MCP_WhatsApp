...
export function MessageThread({
  ...
  const sessionInfo = useMemo(() => {
    ...
    if (!lastCustomerMsg) return { expired: true, remaining: t("inbox.thread.noCustomerMessages") };
    ...
    const remaining =
      hoursLeft >= 1
        ? t("inbox.thread.hoursRemaining").replace("{h}", String(Math.floor(hoursLeft)))
        : t("inbox.thread.minutesRemaining").replace("{m}", String(Math.floor(hoursLeft * 60)));
    ...
    async (text: string, replyToId?: string) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          toast.error(t("inbox.thread.sendFailed").replace("{reason}", reason));
        ...
        const reason = err instanceof Error ? err.message : t("inbox.thread.networkError");
        toast.error(t("inbox.thread.sendFailed").replace("{reason}", reason));
        onUpdateMessage(tempId, { status: "failed" });
      }
    },
    ...
    async (payload: SendMediaPayload) => {
      ...
      const contentText =
        payload.kind === "document"
          ? payload.caption || payload.filename || t("inbox.bubble.document")
          : payload.caption;
    ...
    async (
      ...
      try {
        ...
        if (!res.ok) {
          ...
          toast.error(t("inbox.thread.sendTemplateFailed").replace("{reason}", reason));
          onUpdateMessage(tempId, { status: "failed" });
          return;
        }
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      if (messageId.startsWith("temp-")) {
        toast.error(t("inbox.thread.reactionWaitForSend"));
        return;
      }
      ...
      try {
        ...
        const reason = err instanceof Error ? err.message : t("inbox.thread.networkError");
        toast.error(t("inbox.thread.reactionFailed").replace("{reason}", reason));
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <p className="text-sm text-muted-foreground">{t("inbox.thread.noMessagesYet")}</p>
