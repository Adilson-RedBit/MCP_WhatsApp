...
export default function ContactsPage() {
  ...
  async function handleBulkDelete() {
    ...
    if (error) {
      ...
      toast.success(
        `${ids.length} ${ids.length === 1 ? t('contacts.bulkDeleteToastSingular') : t('contacts.bulkDeleteToastPlural')}`
      );
      setSelected(new Set());
      fetchContacts();
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {t('contacts.subtitle')} {totalCount > 0 && t('contacts.subtitleTotal').replace('{count}', String(totalCount))}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {canEditSettings && (
            ...
            >
              ...
              {t('contacts.customFieldsButton')}
            </Button>
          )}
          <GatedButton
            variant="outline"
            canAct={canEdit}
            gateReason={t('contacts.gateReasonAddImport')}
                  ...
                  {allTags.map((tag) => (
                    ...
                    >
                      <Checkbox
                        ...
                        aria-label={t('contacts.filterByTagAria').replace('{name}', tag.name)}
                      />
                      <span
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    aria-label={t('contacts.removeFilterAria').replace('{name}', tag.name)}
      ...
      {selected.size > 0 && (
        <div className="flex items-center justify-between gap-4 rounded-lg border border-border bg-muted/40 px-4 py-2">
          <p className="text-sm text-foreground">
            ...
            {selected.size === 1 ? t('contacts.selectedSingular') : t('contacts.selectedPlural')}
          </p>
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
            ...
            <GatedButton
              ...
              size="sm"
              canAct={canEdit}
              gateReason={t('contacts.gateReasonDelete')}
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border hover:bg-transparent">
              ...
              <TableHead className="text-muted-foreground hidden lg:table-cell">{t('contacts.table.createdAt')}</TableHead>
              ...
              <TableRow className="border-border">
                <TableCell colSpan={8} className="text-center py-12">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">{t('contacts.table.loading')}</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : contacts.length === 0 ? (
                ...
                <TableCell colSpan={8} className="text-center py-12">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {hasActiveFilters
                        ? t('contacts.table.emptyFiltered')
                        : t('contacts.table.emptyNone')}
                    </p>
                    {!hasActiveFilters && (
                      <Button
                        variant="outline"
                ...
                >
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      ...
                      aria-label={t('contacts.table.selectRowAria').replace('{name}', contact.name || contact.phone)}
                    />
                  </TableCell>
                  <TableCell className="text-foreground font-medium">
                    {contact.name || <span className="text-muted-foreground italic">{t('contacts.table.noName')}</span>}
                  </TableCell>
        ...
        <DialogContent className="bg-popover border-border text-popover-foreground sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              {t('contacts.deleteDialog.descPrefix')}{' '}
              <span className="text-popover-foreground font-medium">
                {deleteTarget?.name || deleteTarget?.phone}
              </span>
              {t('contacts.deleteDialog.descSuffix')}
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="bg-popover border-border text-popover-foreground sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-popover-foreground">
              {t('common.delete')} {selected.size} {selected.size === 1 ? t('contacts.contactSingular') : t('contacts.contactPlural')}
            </DialogTitle>
            <DialogDescription className="text-muted-foreground">
              {t('contacts.deleteDialog.descPrefix')}{' '}
              <span className="text-popover-foreground font-medium">
                {selected.size} {selected.size === 1 ? t('contacts.contactSingular') : t('contacts.contactPlural')}
              </span>
              {t('contacts.deleteDialog.descSuffix')}
            </DialogDescription>
          </DialogHeader>
