...
export default function JoinPage() {
  ...
  const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
    not_found: {
      title: t('join.fail.notFound.title'),
      body: t('join.fail.notFound.body'),
    },
    used: {
      title: t('join.fail.used.title'),
      body: t('join.fail.used.body'),
    },
    expired: {
      title: t('join.fail.expired.title'),
      body: t('join.fail.expired.body'),
    },
    server_error: {
      title: t('join.fail.serverError.title'),
      body: t('join.fail.serverError.body'),
    },
  };
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('join.toast.serverUnreachable'));
      setAccepting(false);
    }
  }, [token, t]);
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {t('join.conflict.titlePrefix')} {peek.account_name}{' '}
                {t('join.conflict.titleSuffix')}
              </DialogTitle>
